...
save_run_provenance() {
  local out="$RESULTS_DIR/provenance.txt"
  {
    echo "timestamp=$(date --iso-8601=seconds)"
    echo "root=$ROOT_DIR"
    echo "compose_files=$COMPOSE_FILES_VALUE"
    echo "build_mode=$BUILD_MODE"
    echo "clean_start=$CLEAN_START"
    echo "keep_stack=$KEEP_STACK"
    echo "native_build_image=$NATIVE_BUILD_IMAGE"
    echo
    echo "--- git"
...
save_round_provenance() {
  local round="$1"
  local out="$RESULTS_DIR/provenance-round-$round.txt"
  local service cid image_id
  {
    ...
    for service in service-1 service-2 load-balancer warmup; do
      ...
      echo "container=$cid"
      echo "image_ref=$(docker inspect -f '{{.Config.Image}}' "$cid" 2>/dev/null || true)"
      echo "image_id=$image_id"
      if [[ -n "$image_id" ]]; then
        docker image inspect "$image_id" \
          --format 'image_created={{.Created}} repo_tags={{.RepoTags}} repo_digests={{.RepoDigests}}' \
          2>/dev/null || true
      fi
      echo "--- env"
      ...
      docker exec "$cid" sh -lc '
        for p in /app/native-server /app/fd-lb /app/main /out/main; do
          if [ -e "$p" ]; then
            sha256sum "$p"
          fi
        done
